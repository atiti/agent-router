...
if [ ! -x "$AGENTROUTE_BIN_DIR/codex-bin" ]; then
    AGENTROUTE_MIN_FREE_KB=${AGENTROUTE_MIN_FREE_KB:-6291456}
    AGENTROUTE_FREE_KB=$(df -Pk "$AGENTROUTE_HOME_DIR" | awk 'NR == 2 { print $4 }')
    if [ "$AGENTROUTE_FREE_KB" -lt "$AGENTROUTE_MIN_FREE_KB" ]; then
        printf 'AgentRoute needs at least 6 GiB free to build Codex safely; only %s MiB is available.\n' \
            "$((AGENTROUTE_FREE_KB / 1024))" >&2
